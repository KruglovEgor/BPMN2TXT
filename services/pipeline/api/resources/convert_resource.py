...
async def convert_image(request: Request):
    """Метод POST для управления преобразованием изображения в соответствующую
    BPMN-модель. Запрос должен содержать поля формы, которые указывают, нужно ли
    выполнять детекцию элементов, детекцию потоков и OCR на изображениях

    Возвращает
    -------
    str
        Преобразованная BPMN-модель
    """
